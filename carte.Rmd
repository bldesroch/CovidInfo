---
title: "Ou se faire vacciner et tester"
...
```
### Le dépistage
Les tests virologiques (RT-PCR) sont effectués dans des centres de dépistage que vous pourrez retrouver sur cette carte.
...
options = leafletOptions(
            attributionControl=FALSE)) %>% addTiles() %>% addMarkers(data = centres_vaccination, lng =   centres_vaccination$long_coor1, lat =   centres_vaccination$lat_coor1,icon=MarkerIcon_vac,  clusterOptions = markerClusterOptions(iconCreateFunction=JS("function (cluster) {var childCount = cluster.getChildCount();  
    if (childCount < 50) {  
