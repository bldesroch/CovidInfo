...
`r format(sum(Tot_dc_dep$Total_décès),scientific=FALSE)`
</div>
...
plot_ly()%>% add_trace(visible=TRUE, x=Tot_rea$jour, y=Tot_rea$Réa, name = 'Réanimation',mode = 'lines', line = list(color=bleu_clair))%>%
  add_trace(visible=FALSE,x=Tot_hosp$jour,y=Tot_hosp$Hosp,name = 'Hospitalisation',mode = 'lines', line = list(color=bleu_moy))%>%
  add_trace(visible=FALSE, x=Tot_dc$jour,y=Tot_dc$Décès, name = 'Décès',mode = 'lines', line = list(color=bleu_fonc))%>%
